...
| 6-04-3 | 04 | 3 | ACCT-01, ACCT-04 | T-06-01 / T-06-04 / T-06-05 / T-06-06 | Mocked Clerk + db: auth → null ⇒ no db call; driver error text never reaches the result; `db.delete(users)` awaited BEFORE `clerkClient().users.deleteUser`, DB failure short-circuits Clerk; guest-flow gate scans `src/components/**` for `@/db`, `drizzle-orm`, `@clerk/nextjs/server`, `CLERK_SECRET_KEY`; `page.tsx` has no `export const dynamic`; `/methodology`, `/privacy`, `/og` never import `@clerk/nextjs/server`; flips `wave_0_complete: true` | unit (TDD, mock call order) + source-scan test + full suite | `npx vitest run tests/account-actions.test.ts tests/guest-flow-gate.test.ts && npx vitest run && npm run typecheck && npm run lint` | ✅ `tests/account-actions.test.ts`, `tests/guest-flow-gate.test.ts` | ⬜ pending |
