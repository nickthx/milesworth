...
| 07-02-01 | 07-02 | 1 | PLAT-05 | T-07-04 | brand.ts + tokens + warm neutrals + pb-safe | CLI | greps + `npm run typecheck` (build once in 07-02-03) | ❌ W1 | ⬜ pending |
...
| 07-04-02 | 07-04 | 2 | PLAT-05/02 | T-07-14 | Island sweep; masthead; min-h-11; pb-safe; credit line | greps + typecheck/lint | greps + `npm run typecheck && npm run lint` (build once in 07-04-03) | ✅ | ⬜ pending |
...
| 07-05-02 | 07-05 | 3 | PLAT-02/05 | T-07-12, T-07-18, T-07-21 | share-link.tsx fallback field; card + teaser images; hint per D7-02 | greps + typecheck/lint | `npm run typecheck && npm run lint` + `grep -rn "window.location.href" src` empty (build once in 07-05-03) | ✅ | ⬜ pending |
| 07-05-03 | 07-05 | 3 | PLAT-02 | T-07-14 | viewport export, no zoom lock; footer credit reconciled with manifest; PLAT-02 pins | unit (source scan) | `npx vitest run tests/design-system-gate.test.ts` | ❌ W1 (07-02) | ⬜ pending |
...
| 07-08-03 | 07-08 | 4 | DATA-04 | T-07-22, T-07-11 | Floor = N; Neon = seed; bookkeeping; footer credit re-check | unit + DB one-liner | `npm run db:seed && … && npm test` | ✅ | ⬜ pending |
...
- [ ] 07-01-03 `config/lighthouserc.cjs` + `"lighthouse"` / `"images:optimize"` npm scripts + `.lighthouseci/`, `src/images/raw/` in `.gitignore`
